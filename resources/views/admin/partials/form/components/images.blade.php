    ...
    <div class="row gutters-sm js-images__wrapper" data-images-type="{{ $type }}" data-images-is-multiple="{{ $multiple }}">
        ...
        @foreach($images as $image)
            <div class="col-2 col-sm-2 js-image">
                <label class="imagecheck mb-4 js-image__label">
                    ...
                    <input class="js-image__path" type="hidden" data-name-before="{{ $name }}[values]" name="{{ $name }}[values][{{ $key }}][path]" data-name-after="[path]" value="{{ $image->path }}">
                    <input class="js-image__is-new" type="hidden" data-name-before="{{ $name }}[values]" name="{{ $name }}[values][{{ $key }}][is_new]" data-name-after="[is_new]" value="0">
        ...
        <div class="col-2 col-sm-2 js-image js-image--clone">
            <label class="imagecheck mb-4 js-image__label">
                ...
                <input class="js-image__path" type="hidden" data-name-before="{{ $name }}[values]" name="{{ $name }}[values][{{ $key }}][path]" data-name-after="[path]" value="" disabled>
                <input class="js-image__is-new" type="hidden" data-name-before="{{ $name }}[values]" name="{{ $name }}[values][{{ $key }}][is_new]" data-name-after="[is_new]" value="1" disabled>
                <a href="#" class="imagecheck-remove js-image__remove"><span class="fe fe-x"></span></a>
            </label>
            <div class="js-image__custom-attributes">
            </div>
